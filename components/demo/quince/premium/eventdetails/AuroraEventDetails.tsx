...
export function AuroraEventDetails() {
  ...
  if (!auroraDemoData.event || !auroraDemoData.event.ceremony || !auroraDemoData.event.party) {
    return (
      <section className="relative py-20 px-4 min-h-screen overflow-hidden">
        <div className="max-w-4xl mx-auto text-center">
          <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded">
            ...
            <br />
            <small>auroraDemoData.event: {JSON.stringify(auroraDemoData.event, null, 2)}</small>
          </div>
        </div>
      </section>
    )
  ...
  return (
    <section className="relative py-20 px-4 min-h-screen overflow-hidden">
      ...
      <div ref={ref} className="relative z-10 max-w-6xl mx-auto text-center opacity-100 translate-y-0">
        ...
        <div className="mb-12">
          <div className="inline-block bg-white/95 backdrop-blur-md px-10 py-6 rounded-2xl border-2 border-aurora-primary/50 shadow-2xl ring-2 ring-aurora-primary/30 relative overflow-hidden">
            <div className="absolute inset-0 aurora-shimmer opacity-25"></div>
            <div className="relative z-10 flex items-center gap-3">
              <Calendar className="w-8 h-8 text-aurora-primary drop-shadow-[0_2px_8px_rgba(180,80,255,0.25)]" />
              <div className="text-left">
                <p className="text-sm text-gray-800 font-semibold">📅 Fecha del Evento</p>
                <p className="text-2xl font-princess aurora-text-gradient font-bold drop-shadow-[0_2px_6px_rgba(180,80,255,0.18)]">
                  {auroraDemoData.event.date.day}
                </p>
                <p className="text-lg text-gray-900 font-medium">{auroraDemoData.event.date.date}</p>
              </div>
            </div>
          </div>
        </div>
