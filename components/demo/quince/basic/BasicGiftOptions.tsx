  ...
  return (
    <section className="py-16 px-4 bg-white">
      ...
      >
        ...
        <p className="text-lg mt-8 mb-12 max-w-2xl mx-auto text-plateado-700">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {basicDemoData.gifts.options.map((option, index) => (
            <div
              key={index}
              className="bg-rosa-gold-50/50 rounded-lg p-6 text-center hover:shadow-lg transition-all duration-300 border border-plateado-200 hover:border-rosa-gold-300 hover:scale-105"
            >
              <div className="text-4xl mb-4">{option.icon}</div>
              <h3 className="text-xl font-medium mb-2 text-rosa-gold-700">{option.title}</h3>
              <p className="text-lg font-medium mb-3 text-rosa-gold-600">{option.description}</p>
              <div className="text-sm text-plateado-600 whitespace-pre-line">
                {option.details}
              </div>
            </div>
          ))}
